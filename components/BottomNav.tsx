...
const tabs = [
  ...
  { id:'about', label:'About', d:'M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm0-9v5m0-8v1' },
];
export default function BottomNav({ active, onChange }: { active: string; onChange: (t: string) => void }) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-lk-card border-t border-lk-border flex py-1">
      {tabs.map(t => {
        const a = active === t.id;
        return (
          <button key={t.id} onClick={() => onChange(t.id)}
            className={`flex-1 flex flex-col items-center gap-0.5 py-2 transition-all min-h-[52px] ${a ? 'text-lk-accent' : 'text-lk-dim'}`}>
            <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
              <path d={t.d}/>
              {t.id === 'profile' && <circle cx="12" cy="7" r="4"/>}
            </svg>
            <span className={`text-[9px] leading-tight ${a ? 'font-semibold' : ''}`}>{t.label}</span>
          </button>
        );
      })}
    </nav>
